...
2. For $k = 1$ to $K$:
    ...
    2. $D_{\text{train}, k} \leftarrow D_{\text{train}} \setminus F_k$  // Remaining folds as training subset
...
3. Combine validation outputs from all $K$ folds into a unified validation result set

4. Define cross-entropy objective function $\mathcal{L}_{\text{CE}}(w)$ based on Eq. B.1, using:
    * Combined validation outputs ($p_{\text{nn}}$, $p_{\text{nli}}$, $c_{\text{llm}}$)
    * Corresponding ground-truth labels $y_{\text{val}}$


5.  Solve for optimal weights by minimizing the objective function:
    ```math
